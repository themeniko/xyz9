    ...
    while True:
        ...
        if response.status_code == 200:   
            try:
                ...
                text_fa = pytesseract.image_to_string(img, lang='ara')
                if is_correct(text_fa):
                    text_en = pytesseract.image_to_string(img)
...
def is_correct(string):
    return any(word in string for word in ['درست', 'همينه', 'حالادر', 'واردش كن', 'صفحه شكار كنج'])
...
#update result
session.get(f"https://my.abdee.ir/?action=log&range={Range_text}&sort={SORT}&result={len(all_product_ids)}&time={time_taken}&ip={ip_address}")
